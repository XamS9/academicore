...
**Tuition amount:** Each subject may have an optional `tuitionAmount` (Decimal). **Open enrollment** (available groups, cart) always uses **`credits × SystemSettings.creditCost`** for displayed totals. **Monthly installments** use **`credits × creditCost`** per enrolled subject by default; if the student has a **PENDING or APPROVED** `tuition_group_request` for that subject in the period, that line uses **`tuitionAmount` when set** (otherwise credits × cost), matching the tutoría request pricing.
...
| Create | `POST /api/periodos` | name, startDate, endDate, enrollmentOpen; optional `enrollmentPhaseStartDate`, `enrollmentPhaseEndDate` (ISO dates) |
| Update | `PATCH /api/periodos/:id` | Dates, name, optional enrollment phase dates |
| Toggle enrollment | `PATCH /api/periodos/:id/toggle-enrollment` | Opens/closes enrollment window |

**Rules:** `enrollmentOpen` gates student enrollment together with optional **enrollment phase dates**: if set, self-service enrollment is only effective between those calendar days (UTC). After the end date, the API sets `enrollmentOpen` to false automatically on the next relevant request (`syncEnrollmentPhaseAutoClose`). List/get responses include `enrollmentOpenEffective` for UI. `isActive` marks the current semester.
...
- **Monthly tuition (matrícula por período):** El sistema ya **no** crea un cargo único de «Matrícula de materia» por cada inscripción a grupo. Por materia inscrita, la base de mensualidades es **`credits × SystemSettings.creditCost`**, salvo que exista solicitud de tutoría **PENDING o APPROVED** para esa materia en el período, en cuyo caso aplica **`tuitionAmount` si está definido** (precio tutoría). Tras cumplir la inscripción al período (cuota de inscripción **pagada** si `inscriptionFee > 0`, o de inmediato si es 0), las **mensualidades** se sincronizan: el total se divide en **`12 ÷ cyclesPerYear`** cuotas (`StudentFee` del concepto «Mensualidad», `installmentNumber` 1…N, vencimientos mensuales desde `academic_period.start_date`). Al inscribir, dar de baja materias o **crear solicitud de tutoría** se **recalculan** las cuotas **pendientes** respetando las ya pagadas.
...
| Credit Cost | `creditCost` | 0 | Global rate per credit. **Open enrollment** display uses `credits × creditCost`. **Monthly installments** sum that per enrolled subject, unless the student has a **PENDING/APPROVED** tutoría request for that subject—then `tuitionAmount` applies when set (`syncMonthlyTuitionInstallments`). |
...
**Monthly tuition installments:** After the inscription requirement is satisfied (`inscriptionFee = 0` or the inscription `StudentFee` is **PAID**), `syncMonthlyTuitionInstallments` materializes **`12 / cyclesPerYear`** pending fees (concept «Mensualidad académica» / name contains `"mensualidad"`). The total sums, per enrolled subject, **`credits × creditCost`** by default, or **`tuitionForSubject`** (fixed `tuitionAmount` when set) when the student has a **PENDING or APPROVED** `tuition_group_request` for that subject in the period. Paying the inscription triggers generation; enroll/drop/**tuition request create** triggers recalculation of **pending** installments (paid installments kept).
...
`GET /api/enrollments/available-groups` returns a `computedTuition` field per group as **`credits × SystemSettings.creditCost` only** (open enrollment ignores `Subject.tuitionAmount`). For **solicitudes de tutoría** (active groups below `minStudentsToOpenGroup` after enrollment closes), `GET /api/enrollments/tuition-request-options` lists career groups under the minimum **and** groups the student is **already enrolled in** that are still under the minimum (`isMyEnrolledGroup`). It returns `computedTuition` using **`Subject.tuitionAmount` when set**, otherwise the same credit-based formula. `POST /api/enrollments/tuition-request` rejects duplicate enrollment in another section of the same subject but **allows** the request when the student is already in the same under-minimum `groupId`. Creating a request triggers **mensualidad resync** so installments include tutoría-priced subjects when applicable. The cart sidebar on `/inscribir` shows:
