...
function parseOptionalPhaseDateInput(value: string | undefined): Date | null {
  if (value == null || value.trim() === "") return null;
  const d = new Date(value);
  if (Number.isNaN(d.getTime())) {
    throw new HttpError(400, "Fecha de fase de inscripción inválida");
  }
  return d;
}

function assertEnrollmentPhaseOrder(start: Date | null, end: Date | null) {
  if (start && end && utcDayNumber(start) > utcDayNumber(end)) {
    throw new HttpError(
      400,
      "La fecha de inicio de inscripciones debe ser anterior o igual a la de cierre automático",
    );
  }
}

function assertEnrollmentOpenMatchesPhaseEnd(
  enrollmentOpen: boolean,
  phaseEnd: Date | null,
  at: Date = new Date(),
) {
  if (enrollmentOpen && phaseEnd && isPastEnrollmentPhaseEnd(phaseEnd, at)) {
    throw new HttpError(
      400,
      "La fecha de cierre de inscripciones ya pasó. Amplía la fecha de cierre o deja inscripciones cerradas.",
    );
  }
}
...
class AcademicPeriodsService {
  ...
  async create(dto: CreateAcademicPeriodDto) {
    const phaseStart = parseOptionalPhaseDateInput(dto.enrollmentPhaseStartDate);
    const phaseEnd = parseOptionalPhaseDateInput(dto.enrollmentPhaseEndDate);
    assertEnrollmentPhaseOrder(phaseStart, phaseEnd);
    assertEnrollmentOpenMatchesPhaseEnd(dto.enrollmentOpen, phaseEnd);
    ...
    return prisma.academicPeriod.create({
      data: {
        ...
        enrollmentPhaseStartDate: phaseStart,
        enrollmentPhaseEndDate: phaseEnd,
      },
    });
  }
  ...
  async update(id: string, dto: UpdateAcademicPeriodDto) {
    ...
    const phaseStart =
      dto.enrollmentPhaseStartDate !== undefined
        ? parseOptionalPhaseDateInput(dto.enrollmentPhaseStartDate)
        : period.enrollmentPhaseStartDate;
    const phaseEnd =
      dto.enrollmentPhaseEndDate !== undefined
        ? parseOptionalPhaseDateInput(dto.enrollmentPhaseEndDate)
        : period.enrollmentPhaseEndDate;
    ...
    const data: {
      ...
      endDate?: Date;
      enrollmentOpen?: boolean;
      enrollmentPhaseStartDate?: Date | null;
      enrollmentPhaseEndDate?: Date | null;
    } = {
    ...
    const nextOpen =
      data.enrollmentOpen !== undefined ? data.enrollmentOpen : period.enrollmentOpen;
    assertEnrollmentOpenMatchesPhaseEnd(nextOpen, phaseEnd);
  ...
  async toggleEnrollment(id: string) {
    ...
    const nextOpen = !period.enrollmentOpen;
    assertEnrollmentOpenMatchesPhaseEnd(nextOpen, period.enrollmentPhaseEndDate);
    return prisma.academicPeriod.update({
      where: { id },
      data: { enrollmentOpen: nextOpen },
    });
  }
