...
export class EnrollmentsController {
  ...
  findTuitionRequestOptions = async (
    req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> => {
    try {
      const groups = await this.service.findTuitionRequestOptions(req.user!.sub);
      res.json(groups);
    } catch (err) {
      next(err);
    }
  ...
  createTuitionRequest = async (
    req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> => {
    try {
      const dto = CreateTuitionRequestDto.parse(req.body);
      const row = await this.service.createTuitionRequest(req.user!.sub, dto);
  ...
  listTuitionRequestsAdmin = async (
    _req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> => {
    try {
      const rows = await this.service.listTuitionRequestsForAdmin();
      res.json(rows);
    } catch (err) {
      next(err);
    }
