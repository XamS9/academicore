...
class CertificationsController {
  findAll = async (
    ...
    try {
      res.json(await certificationsService.findAll());
    } catch (err) {
      next(err);
    }
  };
  ...
  validateByCode = async (
    req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> => {
    try {
      res.json(await certificationsService.validateByCode(req.params.code));
    } catch (err) {
      next(err);
    }
  };
  ...
  revoke = async (
    ...
    try {
      ...
      const cert = await certificationsService.revoke(
        req.params.id,
        dto,
        req.user!.sub,
      );
  ...
  findCriteria = async (
    ...
    try {
      res.json(await certificationsService.findCriteria());
    } catch (err) {
      next(err);
    }
  };
  ...
  downloadPdf = async (
    ...
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="certificado-${req.params.id.slice(0, 8)}.pdf"`,
      );
      res.send(pdf);
    } catch (err) {
      next(err);
