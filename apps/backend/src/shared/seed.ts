 ...
 *
 * E2E (contraseñas: admin123 / teacher123 / student123, alineadas al demo):
 * `teacher.e2e` imparte `ETH101-E2E` (2026-1) con temas, materiales y
 ...
 * `student.prereq.e2e` tiene aprobada MAT101 (2025-2) y **sin inscripciones en
 * 2026-1** — listo para inscribir BD101, PRG201, etc. según prerrequisitos.
...
export async function runSeed(prisma: PrismaClient): Promise<void> {
  ...
  const [
    ...
    prisma.user.upsert({
      where: { email: "student.prereq.e2e@academicore.com" },
      update: {
        isActive: true,
        deletedAt: null,
        passwordHash: await hash("student123"),
      ...
      create: {
        ...
        firstName: "Prereq",
        lastName: "ListoE2E",
        email: "student.prereq.e2e@academicore.com",
        passwordHash: await hash("student123"),
        isActive: true,
      },
    }),
  ...
  await Promise.all([
    ...
    prisma.userRole.upsert({
      where: {
        userId_roleId: { userId: studentPrereqE2EUser.id, roleId: studentRole.id },
      },
      update: {},
      create: { userId: studentPrereqE2EUser.id, roleId: studentRole.id },
    }),
  ]);
  ...
  const studentPrereqE2E = await prisma.student.upsert({
    where: { studentCode: "EST-PRE-E2E-001" },
    update: {
      userId: studentPrereqE2EUser.id,
      careerId: career.id,
      academicStatus: "ACTIVE",
      deletedAt: null,
    },
  ...
  // MAT101 sección dedicada (2025-2) — solo alumno E2E con prereq cumplida, sin solapar `student` en MAT101-A
  const grpMat101PreE2E = await prisma.group.upsert({
    where: {
      subjectId_academicPeriodId_groupCode: {
        subjectId: mat101.id,
        academicPeriodId: period2024.id,
        groupCode: "MAT101-PRE",
      },
    },
    update: {},
    create: {
  ...
  await Promise.all([
    ...
    prisma.academicRecord.upsert({
      where: {
        studentId_groupId: {
          studentId: studentPrereqE2E.id,
          groupId: grpMat101PreE2E.id,
        },
      },
      update: {},
      create: {
  ...
  console.log("");
  console.log("   E2E accounts (misma convención role123 que demo: admin123 / teacher123 / student123):");
  console.log("   Admin:    admin.e2e@academicore.com    / admin123");
  console.log("   Teacher:  teacher.e2e@academicore.com   / teacher123  → grupo ETH101-E2E (2026-1), temas + evaluaciones");
  console.log("   Student:  student.e2e@academicore.com   / student123  → sin materias; cargo inscripción al inscribir primer grupo (2026-1)");
  console.log("   Prereq:   student.prereq.e2e@academicore.com / student123  → MAT101 aprobada (2025-2), sin carga 2026-1; puede inscribir BD101, no PRG201 aún (falta PRG101)");
  console.log("");
