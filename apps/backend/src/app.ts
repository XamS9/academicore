...
// Global middleware — allow embedding static files (e.g. /api/files) in the SPA when
// frontend and API are different origins (Vite dev, or split CDN + API). Helmet's default
// Cross-Origin-Resource-Policy is same-origin, which blocks <img src> from another origin.
app.use(
  helmet({
