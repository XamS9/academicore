...
import { formatPrerequisitesLine, listPrerequisiteSubjects } from "../components/SubjectPrerequisitesCaption";
...
interface UpcomingDeadline {
  ...
  subjectName: string;
  /** Prerequisite subjects for the course (codes and names). */
  prerequisitesLine?: string;
  dueDate: string;
  submitted: boolean;
  daysLeft: number;
}
  ...
  const evalsByGroup = await Promise.all(
    activeGroups.map((g) =>
      (evaluationsService.getByGroup(g.groupId) as Promise<EvalItem[]>)
        .then((evs) =>
          evs.map((ev) => {
            const pl = formatPrerequisitesLine(listPrerequisiteSubjects(g.subject));
            return {
              ...ev,
              subjectName: g.subjectName,
              prerequisitesLine: pl || undefined,
            };
          }),
        )
        .catch(
  ...
  const upcoming: UpcomingDeadline[] = evalsByGroup
    ...
    .map((ev) => {
      ...
      return {
        ...
        prerequisitesLine: ev.prerequisitesLine,
        dueDate: ev.dueDate!,
                              ...
                              secondary={
                                <Box>
                                  <Typography variant="caption" color="text.secondary" display="block">
                                    {d.subjectName} ·{" "}
                                    {new Date(d.dueDate).toLocaleDateString("es", {
                                      day: "numeric",
                                      month: "short",
                                    })}
                                  </Typography>
                                  {d.prerequisitesLine ? (
                                    <Typography
                                      ...
                                      display="block"
                                    >
                                      Prerrequisitos: {d.prerequisitesLine}
                                    </Typography>
                                  ) : null}
                                </Box>
                              }
