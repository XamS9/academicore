  ...
  const openSubmissions = async (ev: EvaluationItem) => {
    ...
    try {
      ...
      (existingGrades as GradeRow[]).forEach((g) => {
        scoreMap[g.student.id] = String(Number(g.score));
      });
      ...
      >
        ...
        <DialogContent className="flex flex-col gap-4 pt-4">
          ...
          <NumericTextField
            label="Peso (%)"
            value={form.weight}
            onValueChange={(weight) => setForm({ ...form, weight: Math.max(0, weight) })}
            min={0}
            max={100}
          ...
          <NumericTextField
            ...
            value={form.maxScore}
            onValueChange={(maxScore) => setForm({ ...form, maxScore: Math.max(1, maxScore) })}
            min={1}
            inputProps={{ min: 1 }}
            fullWidth
            margin="dense"
