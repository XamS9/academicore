...
export default function ReportsPage() {
  ...
  const [atRisk, setAtRisk] = useState<{ count: number; students: any[] }>({ count: 0, students: [] });
  ...
  if (loading) {
    return <Box className="flex justify-center py-12"><CircularProgress /></Box>;
  }
  ...
  return (
    <Box>
      <Typography variant="h5" sx={{ mb: 3 }}>Reportes y Analíticas</Typography>

      <Grid container spacing={3}>
        {/* Enrollment Stats */}
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 2 }}>
            <Typography variant="subtitle1" fontWeight={600} sx={{ mb: 2 }}>Inscripciones por Período</Typography>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 2 }}>
            <Typography variant="subtitle1" fontWeight={600} sx={{ mb: 2 }}>Aprobados vs Reprobados</Typography>
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 2 }}>
            <Typography variant="subtitle1" fontWeight={600} sx={{ mb: 2 }}>Tendencia de Promedio General</Typography>
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={gpaTrends}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="averageGpa" stroke="#1976d2" name="Promedio" strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </Paper>
        </Grid>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 2 }}>
            <Typography variant="subtitle1" fontWeight={600} sx={{ mb: 2 }}>Estudiantes en Riesgo</Typography>
            <Typography variant="h2" color="error" fontWeight={700} sx={{ mb: 1 }}>{atRisk.count}</Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>estudiantes con estatus AT_RISK</Typography>
