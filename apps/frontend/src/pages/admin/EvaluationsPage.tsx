    ...
    const loadInitial = async () => {
      try {
        const [types, groupList] = await Promise.all([
          ...
          isTeacher
            ? teachersService.getByUserId(currentUser!.id).then((t: { id: string }) => groupsService.getByTeacher(t.id))
    ...
    },
    {
      ...
      render: (row) => (
        <>
          ...
          <IconButton size="small" onClick={() => handleDelete(row.id)} title="Eliminar" color="error">
  ...
  return (
    <Box>
      <Box className="flex justify-between items-center mb-6">
        ...
        <Button variant="contained" onClick={openCreate} disabled={!selectedGroup}>
          Nueva Evaluación
        </Button>
      </Box>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent className="flex flex-col gap-4 pt-4">
          ...
          <TextField
            ...
            type="number"
            value={form.weight}
            onChange={(e) => setForm({ ...form, weight: Math.max(0, Number(e.target.value)) })}
          ...
          <TextField
            ...
            type="number"
            value={form.maxScore}
            onChange={(e) => setForm({ ...form, maxScore: Math.max(1, Number(e.target.value)) })}
            inputProps={{ min: 1 }}
            fullWidth
            margin="dense"
          />
