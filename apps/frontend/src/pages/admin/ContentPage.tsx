...
export default function ContentPage() {
  ...
  const loadTopics = async (groupId: string) => {
    try {
      setLoading(true);
  ...
  const openCreateItem = (topicId: string, currentCount: number) => {
    setEditItem(null);
    setItemForm({ ...emptyItemForm, topicId, sortOrder: currentCount + 1 });
          ...
          <Box sx={{ display: "flex", gap: 2 }}>
            <TextField
              ...
              onChange={(e) => {
                ...
                if (v === "" || /^\d*$/.test(v)) {
                  setTopicForm({ ...topicForm, weekNumber: v });
                }
              }}
              inputProps={{ min: 1 }}
              fullWidth
            ...
            <NumericTextField
              ...
              onValueChange={(sortOrder) =>
                setTopicForm({ ...topicForm, sortOrder: Math.max(1, sortOrder) })
              }
              min={1}
              integer
              inputProps={{ min: 1 }}
      ...
      <Dialog open={itemDialogOpen} onClose={() => setItemDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ display: "flex", flexDirection: "column", gap: 1, pt: 2 }}>
          ...
          <NumericTextField
            ...
            onValueChange={(sortOrder) =>
              setItemForm({ ...itemForm, sortOrder: Math.max(1, sortOrder) })
            }
            min={1}
            integer
            inputProps={{ min: 1 }}
