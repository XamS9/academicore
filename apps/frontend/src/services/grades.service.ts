...
export const gradesService = {
  ...
  getByEvaluation: (evaluationId: string) => api.get(`/grades/evaluation/${evaluationId}`).then(r => r.data),
  getByStudentAndGroup: (studentId: string, groupId: string) =>
    api.get(`/grades/student/${studentId}/group/${groupId}`).then(r => r.data),
  ...
  bulkUpsert: (grades: unknown[]) => api.post('/grades/bulk', { grades }).then(r => r.data),
